...
}, getHostListenerTarget = (e, t) => 8 & t ? win$1 : e, hostListenerOpts = e => 0 != (2 & e), XLINK_NS = "http://www.w3.org/1999/xlink";
...
const createTime = (e, t = "") => {
 ...
 if (o !== n) {
  ...
  } else t = "-" === t[2] ? t.slice(3) : isMemberInElement(win$1, r) ? r.slice(2) : r[2] + t.slice(3), 
...
const callRender = (e, t, o) => {
 ...
 addHydratedFlag(doc.documentElement), nextTick((() => emitEvent(win$1, "appload", {
  detail: {
   namespace: NAMESPACE
  }
 }))), BUILD.profile  ;
...
}, win$1 = window, doc = win$1.document, writeTask = e => {
 process.nextTick((() => {
  try {
   e();
  } catch (e) {
...
function createStore(defaultState) {
  return createStore$1(defaultState);
}
...
const addonsAPI = {
  registerAddon: (addon) => {
    const addons = Object.assign({}, state.addons);
    addons[addon.id] = addon;
    state.addons = addons;
  },
  unregisterAddon: (addon) => {
    state.addons = Object.keys(state.addons).filter(id => id !== addon.id).reduce((addons, id) => {
      addons[id] = state.addons[id];
      return addons;
    }, {});
  },
  getPanels() {
    return Object.keys(state.addons).filter(addonId => state.addons[addonId].type === 'panel').reduce((addons, id) => {
      addons[id] = state.addons[id];
      return addons;
    }, {});
  },
  getTools() {
    return Object.keys(state.addons).filter(addonId => state.addons[addonId].type === 'tool').reduce((addons, id) => {
      addons[id] = state.addons[id];
      return addons;
    }, {});
  },
  ...
  setAddonState(addonId, addonState) {
    const addon = Object.assign({}, state.addons[addonId]);
    addon.state = addonState;
    state.addons = Object.assign(Object.assign({}, state.addons), { [addonId]: addon });
  },
};
...
var story = /*#__PURE__*/Object.freeze({
  __proto__: null
});

var _story = /*@__PURE__*/getAugmentedNamespace(story);
 ...
 */
function modulesToStories(modules) {
  const result = {};
 ...
 * }
 */
function prepareStoryProperties(storyKey, name, title) {
  const kind = title || name || storyKey;
  const storyKind = dist.parseKind(kind, { rootSeparator: '/', groupSeparator: '/' });
  const countedKind = [storyKind.root, ...storyKind.groups].filter(value => !!value).join('-');
  const moduleId = dist.sanitize(countedKind);
  const storyExportName = dist.storyNameFromExport(storyKey);
  const storyName = name || storyExportName;
  const storyId = dist.toId(moduleId, storyName);
 ...
 */
function createStoryComponent(storyId, storyName, storyKind, story, meta) {
  return {
    storyId,
    ...
    decorators: cjs(meta.decorators || [], story.decorators || []),
    ...
    args: cjs(meta.args || {}, story.args || {}),
    argTypes: cjs(meta.argTypes || {}, story.argTypes || {}),
    ...
    // If an element at the same key is present for both story and meta, the value from story will appear in the result.
    // Merging creates a new object, so that neither story or meta is modified.
    parameters: cjs(meta.parameters || {}, story.parameters || {}),
  };
}
/**
...
const target = {};
const api = Object.assign(target, storiesAPI, addonsAPI, layoutAPI, notificationsAPI);
...
const addonActionsCss = "/*!@:host*/.sc-stories-addon-actions-h{display:block;height:100%}";
...
class AddonActions {
  ...
  render() {
    return (hAsync(Host, null, hAsync("ul", null, this.actions.map(action => hAsync("li", null, action.data.name)))));
  }
  get el() { return getElement(this); }
  static get style() { return addonActionsCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-addon-actions",
    "$members$": {
      "actions": [32],
...
const addonControlsCss = "/*!@:host*/.sc-stories-addon-controls-h{display:block}";
...
class AddonControls {
  ...
  render() {
    return (hAsync(Host, null, hAsync("div", null, "Addon Controls")));
  }
  get el() { return getElement(this); }
  static get style() { return addonControlsCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-addon-controls",
    "$members$": {
      "controls": [32],
      "storyContextChanged": [64]
    },
...
const appCss = "/*!@:host*/.sc-stories-app-h{height:100%;width:100%;background-color:coral}";
...
class App {
  ...
  static get cmpMeta() { return {
    ...
    "$listeners$": [[8, "hashchange", "onHash"]],
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
  }; }
}

const badgeCss = "/*!@:host*/.sc-stories-badge-h{--background:var(--stories-color-primary, #3880ff);--color:var(--stories-color-primary-contrast, #fff);--padding-top:3px;--padding-end:8px;--padding-bottom:3px;--padding-start:8px;-moz-osx-font-smoothing:grayscale;-webkit-font-smoothing:antialiased;padding-left:var(--padding-start);padding-right:var(--padding-end);padding-top:var(--padding-top);padding-bottom:var(--padding-bottom);display:inline-block;min-width:10px;background:var(--background);color:var(--color);font-family:var(--stories-font-family, inherit);font-size:13px;font-weight:bold;line-height:1;text-align:center;white-space:nowrap;contain:content;vertical-align:baseline}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@:host*/.sc-stories-badge-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--padding-start);padding-inline-start:var(--padding-start);-webkit-padding-end:var(--padding-end);padding-inline-end:var(--padding-end)}}/*!@:host(.stories-color)*/.stories-color.sc-stories-badge-h{background:var(--stories-color-base);color:var(--stories-color-contrast)}/*!@:host(:empty)*/.sc-stories-badge-h:empty{display:none}";
...
class Badge {
  ...
  render() {
    return (hAsync(Host, { class: createColorClasses(this.color, {}) }, hAsync("slot", null)));
  }
  static get style() { return badgeCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-badge",
    "$members$": {
      "color": [513]
    },
    "$listeners$": undefined,
...
const buttonCss = "/*!@:host*/.sc-stories-button-h{--overflow:hidden;--ripple-color:currentColor;--border-width:initial;--border-color:initial;--border-style:initial;--color-activated:var(--color);--color-focused:var(--color);--color-hover:var(--color);--box-shadow:none;display:inline-block;width:auto;color:var(--color);font-family:var(--stories-font-family, inherit);text-align:center;text-decoration:none;text-overflow:ellipsis;white-space:nowrap;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;vertical-align:top;vertical-align:-webkit-baseline-middle;-webkit-font-kerning:none;font-kerning:none}/*!@:host(.button-disabled)*/.button-disabled.sc-stories-button-h{cursor:default;opacity:0.5;pointer-events:none}/*!@:host(.button-solid)*/.button-solid.sc-stories-button-h{--background:var(--stories-color-primary, #3880ff);--color:var(--stories-color-primary-contrast, #fff)}/*!@:host(.button-outline)*/.button-outline.sc-stories-button-h{--border-color:var(--stories-color-primary, #3880ff);--background:transparent;--color:var(--stories-color-primary, #3880ff)}/*!@:host(.button-clear)*/.button-clear.sc-stories-button-h{--border-width:0;--background:transparent;--color:var(--stories-color-primary, #3880ff)}/*!@:host(.button-block)*/.button-block.sc-stories-button-h{display:block}/*!@:host(.button-block) .button-native*/.button-block.sc-stories-button-h .button-native.sc-stories-button{margin-left:0;margin-right:0;display:block;width:100%;clear:both;contain:content}/*!@:host(.button-block) .button-native::after*/.button-block.sc-stories-button-h .button-native.sc-stories-button::after{clear:both}/*!@:host(.button-full)*/.button-full.sc-stories-button-h{display:block}/*!@:host(.button-full) .button-native*/.button-full.sc-stories-button-h .button-native.sc-stories-button{margin-left:0;margin-right:0;display:block;width:100%;contain:content}/*!@:host(.button-full:not(.button-round)) .button-native*/.button-full.sc-stories-button-h:not(.button-round) .button-native.sc-stories-button{border-radius:0;border-right-width:0;border-left-width:0}/*!@.button-native*/.button-native.sc-stories-button{border-radius:var(--border-radius);-moz-osx-font-smoothing:grayscale;-webkit-font-smoothing:antialiased;margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;padding-left:var(--padding-start);padding-right:var(--padding-end);padding-top:var(--padding-top);padding-bottom:var(--padding-bottom);font-family:inherit;font-size:inherit;font-style:inherit;font-weight:inherit;letter-spacing:inherit;text-decoration:inherit;text-indent:inherit;text-overflow:inherit;text-transform:inherit;text-align:inherit;white-space:inherit;color:inherit;display:block;position:relative;width:100%;height:100%;-webkit-transition:var(--transition);transition:var(--transition);border-width:var(--border-width);border-style:var(--border-style);border-color:var(--border-color);outline:none;background:var(--background);line-height:1;-webkit-box-shadow:var(--box-shadow);box-shadow:var(--box-shadow);contain:layout style;cursor:pointer;opacity:var(--opacity);overflow:var(--overflow);z-index:0;-webkit-box-sizing:border-box;box-sizing:border-box;-webkit-appearance:none;-moz-appearance:none;appearance:none}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@.button-native*/.button-native.sc-stories-button{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--padding-start);padding-inline-start:var(--padding-start);-webkit-padding-end:var(--padding-end);padding-inline-end:var(--padding-end)}}/*!@.button-native::-moz-focus-inner*/.button-native.sc-stories-button::-moz-focus-inner{border:0}/*!@.button-inner*/.button-inner.sc-stories-button{display:-ms-flexbox;display:flex;position:relative;-ms-flex-flow:row nowrap;flex-flow:row nowrap;-ms-flex-negative:0;flex-shrink:0;-ms-flex-align:center;align-items:center;-ms-flex-pack:center;justify-content:center;width:100%;height:100%;z-index:1}/*!@::slotted(stories-icon)*/.sc-stories-button-s>stories-icon{font-size:1.4em;pointer-events:none}/*!@::slotted(stories-icon[slot=start])*/.sc-stories-button-s>stories-icon[slot=start]{margin-left:-0.3em;margin-right:0.3em;margin-top:0;margin-bottom:0}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@::slotted(stories-icon[slot=start])*/.sc-stories-button-s>stories-icon[slot=start]{margin-left:unset;margin-right:unset;-webkit-margin-start:-0.3em;margin-inline-start:-0.3em;-webkit-margin-end:0.3em;margin-inline-end:0.3em}}/*!@::slotted(stories-icon[slot=end])*/.sc-stories-button-s>stories-icon[slot=end]{margin-left:0.3em;margin-right:-0.2em;margin-top:0;margin-bottom:0}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@::slotted(stories-icon[slot=end])*/.sc-stories-button-s>stories-icon[slot=end]{margin-left:unset;margin-right:unset;-webkit-margin-start:0.3em;margin-inline-start:0.3em;-webkit-margin-end:-0.2em;margin-inline-end:-0.2em}}/*!@::slotted(stories-icon[slot=icon-only])*/.sc-stories-button-s>stories-icon[slot=icon-only]{font-size:1.8em}/*!@stories-ripple-effect*/stories-ripple-effect.sc-stories-button{color:var(--ripple-color)}/*!@.button-native::after*/.button-native.sc-stories-button::after{left:0;right:0;top:0;bottom:0;position:absolute;content:\"\";opacity:0}/*!@:host(.stories-activated)*/.stories-activated.sc-stories-button-h{color:var(--color-activated)}/*!@:host(.stories-activated) .button-native::after*/.stories-activated.sc-stories-button-h .button-native.sc-stories-button::after{background:var(--background-activated);opacity:var(--background-activated-opacity)}/*!@:host(.stories-focused)*/.stories-focused.sc-stories-button-h{color:var(--color-focused)}/*!@:host(.stories-focused) .button-native::after*/.stories-focused.sc-stories-button-h .button-native.sc-stories-button::after{background:var(--background-focused);opacity:var(--background-focused-opacity)}@media (any-hover: hover){/*!@:host(:hover)*/.sc-stories-button-h:hover{color:var(--color-hover)}/*!@:host(:hover) .button-native::after*/.sc-stories-button-h:hover .button-native.sc-stories-button::after{background:var(--background-hover);opacity:var(--background-hover-opacity)}}/*!@:host(.button-solid.stories-color) .button-native*/.button-solid.stories-color.sc-stories-button-h .button-native.sc-stories-button{background:var(--stories-color-base);color:var(--stories-color-contrast)}/*!@:host(.button-outline.stories-color) .button-native*/.button-outline.stories-color.sc-stories-button-h .button-native.sc-stories-button{border-color:var(--stories-color-base);background:transparent;color:var(--stories-color-base)}/*!@:host(.button-clear.stories-color) .button-native*/.button-clear.stories-color.sc-stories-button-h .button-native.sc-stories-button{background:transparent;color:var(--stories-color-base)}/*!@:host(.in-toolbar:not(.stories-color):not(.in-toolbar-color)) .button-native*/.in-toolbar.sc-stories-button-h:not(.stories-color):not(.in-toolbar-color) .button-native.sc-stories-button{color:var(--stories-toolbar-color, var(--color))}/*!@:host(.button-outline.in-toolbar:not(.stories-color):not(.in-toolbar-color)) .button-native*/.button-outline.in-toolbar.sc-stories-button-h:not(.stories-color):not(.in-toolbar-color) .button-native.sc-stories-button{border-color:var(--stories-toolbar-color, var(--color, var(--border-color)))}/*!@:host(.button-solid.in-toolbar:not(.stories-color):not(.in-toolbar-color)) .button-native*/.button-solid.in-toolbar.sc-stories-button-h:not(.stories-color):not(.in-toolbar-color) .button-native.sc-stories-button{background:var(--stories-toolbar-color, var(--background));color:var(--stories-toolbar-background, var(--color))}";
...
class Button {
  ...
  render() {
    ...
    return (hAsync(Host, { "aria-disabled": disabled ? 'true' : null, class: createColorClasses(color, {
        [buttonType]: true,
      ...
      }), onClick: this.handleClick }, hAsync(TagType, Object.assign({}, attrs, { class: "button-native", disabled: disabled, onBlur: this.onBlur, onFocus: this.onFocus, part: "native" }, inheritedAttributes), hAsync("span", { class: "button-inner" }, hAsync("slot", { name: "icon-only" }), hAsync("slot", { name: "start" }), hAsync("slot", null), hAsync("slot", { name: "end" })))));
  }
  get el() { return getElement(this); }
  static get style() { return buttonCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-button",
    "$members$": {
      "color": [513],
...
const buttonsCss = "/*!@:host*/.sc-stories-buttons-h{display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;-webkit-transform:translateZ(0);transform:translateZ(0);z-index:99}/*!@::slotted(*) stories-button*/.sc-stories-buttons-s stories-button{--padding-top:0;--padding-bottom:0;margin-left:0;margin-right:0;margin-top:0;margin-bottom:0}";
...
const checkboxCss = "/*!@:host*/.sc-stories-checkbox-h{--background-checked:var(--stories-color-primary, #3880ff);--border-color-checked:var(--stories-color-primary, #3880ff);--checkmark-color:var(--stories-color-primary-contrast, #fff);--checkmark-width:1;--transition:none;display:inline-block;position:relative;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;z-index:2}/*!@:host(.stories-color)*/.stories-color.sc-stories-checkbox-h{--background-checked:var(--stories-color-base);--border-color-checked:var(--stories-color-base);--checkmark-color:var(--stories-color-contrast)}/*!@label*/label.sc-stories-checkbox{left:0;top:0;margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;position:absolute;width:100%;height:100%;border:0;background:transparent;cursor:pointer;-webkit-appearance:none;-moz-appearance:none;appearance:none;outline:none;display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;opacity:0}/*!@[dir=rtl] label, :host-context([dir=rtl]) label*/[dir=rtl].sc-stories-checkbox label.sc-stories-checkbox,[dir=rtl].sc-stories-checkbox-h label.sc-stories-checkbox,[dir=rtl] .sc-stories-checkbox-h label.sc-stories-checkbox{left:unset;right:unset;right:0}/*!@label::-moz-focus-inner*/label.sc-stories-checkbox::-moz-focus-inner{border:0}/*!@input*/input.sc-stories-checkbox{position:absolute;top:0;left:0;right:0;bottom:0;width:100%;height:100%;margin:0;padding:0;border:0;outline:0;clip:rect(0 0 0 0);opacity:0;overflow:hidden;-webkit-appearance:none;-moz-appearance:none}/*!@.checkbox-icon*/.checkbox-icon.sc-stories-checkbox{border-radius:var(--border-radius);display:block;position:relative;width:100%;height:100%;-webkit-transition:var(--transition);transition:var(--transition);border-width:var(--border-width);border-style:var(--border-style);border-color:var(--border-color);background:var(--background);-webkit-box-sizing:border-box;box-sizing:border-box}/*!@.checkbox-icon path*/.checkbox-icon.sc-stories-checkbox path.sc-stories-checkbox{fill:none;stroke:var(--checkmark-color);stroke-width:var(--checkmark-width);opacity:0}/*!@:host(.checkbox-checked) .checkbox-icon,\n:host(.checkbox-indeterminate) .checkbox-icon*/.checkbox-checked.sc-stories-checkbox-h .checkbox-icon.sc-stories-checkbox,.checkbox-indeterminate.sc-stories-checkbox-h .checkbox-icon.sc-stories-checkbox{border-color:var(--border-color-checked);background:var(--background-checked)}/*!@:host(.checkbox-checked) .checkbox-icon path,\n:host(.checkbox-indeterminate) .checkbox-icon path*/.checkbox-checked.sc-stories-checkbox-h .checkbox-icon.sc-stories-checkbox path.sc-stories-checkbox,.checkbox-indeterminate.sc-stories-checkbox-h .checkbox-icon.sc-stories-checkbox path.sc-stories-checkbox{opacity:1}/*!@:host(.checkbox-disabled)*/.checkbox-disabled.sc-stories-checkbox-h{pointer-events:none}";
...
class Checkbox {
  ...
  render() {
    ...
    return (hAsync(Host, { "aria-checked": `${checked}`, "aria-hidden": disabled ? 'true' : null, "aria-labelledby": label ? labelId : null, class: createColorClasses(color, {
        'in-item': hostContext('stories-item', el),
        'checkbox-checked': checked,
        'checkbox-disabled': disabled,
        'checkbox-indeterminate': indeterminate,
        'interactive': true
      }), onClick: this.onClick, role: "checkbox" }, hAsync("svg", { class: "checkbox-icon", viewBox: "0 0 24 24", part: "container" }, path), hAsync("label", { htmlFor: inputId }, labelText), hAsync("input", { ref: focusEl => this.focusEl = focusEl, "aria-checked": `${checked}`, disabled: disabled, id: inputId, onBlur: () => this.onBlur(), onFocus: () => this.onFocus(), type: "checkbox" })));
  }
  get el() { return getElement(this); }
...
const colCss = "/*!@:host*/.sc-stories-col-h{padding-left:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px));padding-right:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px));padding-top:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px));padding-bottom:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px));margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;-webkit-box-sizing:border-box;box-sizing:border-box;position:relative;-ms-flex-preferred-size:0;flex-basis:0;-ms-flex-positive:1;flex-grow:1;width:100%;max-width:100%;min-height:1px}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@:host*/.sc-stories-col-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px));padding-inline-start:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px));-webkit-padding-end:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px));padding-inline-end:var(--stories-grid-column-padding-xs, var(--stories-grid-column-padding, 5px))}}@media (min-width: 576px){/*!@:host*/.sc-stories-col-h{padding-left:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px));padding-right:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px));padding-top:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px));padding-bottom:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-col (margin-inline-start.sc-stories-col: 0).sc-stories-col or.sc-stories-col (-webkit-margin-start.sc-stories-col: 0).sc-stories-col{.sc-stories-col-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px));padding-inline-start:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px));-webkit-padding-end:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px));padding-inline-end:var(--stories-grid-column-padding-sm, var(--stories-grid-column-padding, 5px))}}}@media (min-width: 768px){/*!@:host*/.sc-stories-col-h{padding-left:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px));padding-right:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px));padding-top:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px));padding-bottom:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-col (margin-inline-start.sc-stories-col: 0).sc-stories-col or.sc-stories-col (-webkit-margin-start.sc-stories-col: 0).sc-stories-col{.sc-stories-col-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px));padding-inline-start:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px));-webkit-padding-end:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px));padding-inline-end:var(--stories-grid-column-padding-md, var(--stories-grid-column-padding, 5px))}}}@media (min-width: 992px){/*!@:host*/.sc-stories-col-h{padding-left:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px));padding-right:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px));padding-top:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px));padding-bottom:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-col (margin-inline-start.sc-stories-col: 0).sc-stories-col or.sc-stories-col (-webkit-margin-start.sc-stories-col: 0).sc-stories-col{.sc-stories-col-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px));padding-inline-start:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px));-webkit-padding-end:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px));padding-inline-end:var(--stories-grid-column-padding-lg, var(--stories-grid-column-padding, 5px))}}}@media (min-width: 1200px){/*!@:host*/.sc-stories-col-h{padding-left:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px));padding-right:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px));padding-top:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px));padding-bottom:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-col (margin-inline-start.sc-stories-col: 0).sc-stories-col or.sc-stories-col (-webkit-margin-start.sc-stories-col: 0).sc-stories-col{.sc-stories-col-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px));padding-inline-start:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px));-webkit-padding-end:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px));padding-inline-end:var(--stories-grid-column-padding-xl, var(--stories-grid-column-padding, 5px))}}}";
...
class Col {
  ...
  render() {
    ...
    return (hAsync(Host, { style: Object.assign(Object.assign(Object.assign(Object.assign({}, this.calculateOffset(isRTL)), this.calculatePull(isRTL)), this.calculatePush(isRTL)), this.calculateSize()) }, hAsync("slot", null)));
  }
...
const footerCss = "/*!@stories-footer*/stories-footer.sc-stories-footer{display:block;position:relative;-ms-flex-order:1;order:1;width:100%;z-index:10}/*!@stories-footer stories-toolbar:last-of-type*/stories-footer.sc-stories-footer stories-toolbar.sc-stories-footer:last-of-type{padding-bottom:var(--stories-safe-area-bottom, 0)}";
...
class Footer {
  ...
  render() {
    return (hAsync(Host, { role: "contentinfo" }, hAsync("slot", null)));
  }
  static get style() { return footerCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-footer",
    "$members$": undefined,
    "$listeners$": undefined,
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
...
const gridCss = "/*!@:host*/.sc-stories-grid-h{padding-left:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px));padding-right:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px));padding-top:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px));padding-bottom:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px));margin-left:auto;margin-right:auto;display:block;-ms-flex:1;flex:1}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@:host*/.sc-stories-grid-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px));padding-inline-start:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px));-webkit-padding-end:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px));padding-inline-end:var(--stories-grid-padding-xs, var(--stories-grid-padding, 5px))}}@media (min-width: 576px){/*!@:host*/.sc-stories-grid-h{padding-left:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px));padding-right:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px));padding-top:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px));padding-bottom:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-grid (margin-inline-start.sc-stories-grid: 0).sc-stories-grid or.sc-stories-grid (-webkit-margin-start.sc-stories-grid: 0).sc-stories-grid{.sc-stories-grid-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px));padding-inline-start:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px));-webkit-padding-end:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px));padding-inline-end:var(--stories-grid-padding-sm, var(--stories-grid-padding, 5px))}}}@media (min-width: 768px){/*!@:host*/.sc-stories-grid-h{padding-left:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px));padding-right:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px));padding-top:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px));padding-bottom:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-grid (margin-inline-start.sc-stories-grid: 0).sc-stories-grid or.sc-stories-grid (-webkit-margin-start.sc-stories-grid: 0).sc-stories-grid{.sc-stories-grid-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px));padding-inline-start:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px));-webkit-padding-end:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px));padding-inline-end:var(--stories-grid-padding-md, var(--stories-grid-padding, 5px))}}}@media (min-width: 992px){/*!@:host*/.sc-stories-grid-h{padding-left:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px));padding-right:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px));padding-top:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px));padding-bottom:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-grid (margin-inline-start.sc-stories-grid: 0).sc-stories-grid or.sc-stories-grid (-webkit-margin-start.sc-stories-grid: 0).sc-stories-grid{.sc-stories-grid-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px));padding-inline-start:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px));-webkit-padding-end:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px));padding-inline-end:var(--stories-grid-padding-lg, var(--stories-grid-padding, 5px))}}}@media (min-width: 1200px){/*!@:host*/.sc-stories-grid-h{padding-left:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px));padding-right:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px));padding-top:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px));padding-bottom:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px))}/*!@@supports (margin-inline-start: 0) or (-webkit-margin-start: 0)*/@supports .sc-stories-grid (margin-inline-start.sc-stories-grid: 0).sc-stories-grid or.sc-stories-grid (-webkit-margin-start.sc-stories-grid: 0).sc-stories-grid{.sc-stories-grid-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px));padding-inline-start:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px));-webkit-padding-end:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px));padding-inline-end:var(--stories-grid-padding-xl, var(--stories-grid-padding, 5px))}}}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@:host*/.sc-stories-grid-h{margin-left:unset;margin-right:unset;-webkit-margin-start:auto;margin-inline-start:auto;-webkit-margin-end:auto;margin-inline-end:auto}}/*!@:host(.grid-fixed)*/.grid-fixed.sc-stories-grid-h{width:var(--stories-grid-width-xs, var(--stories-grid-width, 100%));max-width:100%}@media (min-width: 576px){/*!@:host(.grid-fixed)*/.grid-fixed.sc-stories-grid-h{width:var(--stories-grid-width-sm, var(--stories-grid-width, 540px))}}@media (min-width: 768px){/*!@:host(.grid-fixed)*/.grid-fixed.sc-stories-grid-h{width:var(--stories-grid-width-md, var(--stories-grid-width, 720px))}}@media (min-width: 992px){/*!@:host(.grid-fixed)*/.grid-fixed.sc-stories-grid-h{width:var(--stories-grid-width-lg, var(--stories-grid-width, 960px))}}@media (min-width: 1200px){/*!@:host(.grid-fixed)*/.grid-fixed.sc-stories-grid-h{width:var(--stories-grid-width-xl, var(--stories-grid-width, 1140px))}}/*!@:host(.stories-no-padding)*/.stories-no-padding.sc-stories-grid-h{--stories-grid-column-padding:0;--stories-grid-column-padding-xs:0;--stories-grid-column-padding-sm:0;--stories-grid-column-padding-md:0;--stories-grid-column-padding-lg:0;--stories-grid-column-padding-xl:0}";
...
class Grid {
  ...
  render() {
    return (hAsync(Host, { class: { 'grid-fixed': this.fixed } }, hAsync("slot", null)));
  }
  static get style() { return gridCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-grid",
    "$members$": {
      "fixed": [4]
    },
    "$listeners$": undefined,
...
const iconCss = "/*!@:host*/.sc-stories-icon-h{display:inline}/*!@svg*/svg.sc-stories-icon{width:14px;height:14px}/*!@path*/path.sc-stories-icon{fill:currentColor}";
...
class Icon {
  ...
  render() {
    return (hAsync("svg", { viewBox: "0 0 1024 1024" }, hAsync("path", { d: icons[this.name] })));
  }
  static get style() { return iconCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-icon",
    "$members$": {
      "name": [1]
    },
    "$listeners$": undefined,
...
const inputCss = "/*!@:host*/.sc-stories-input-h{--placeholder-color:initial;--placeholder-font-style:initial;--placeholder-font-weight:initial;--placeholder-opacity:.5;--padding-top:0;--padding-end:0;--padding-bottom:0;--padding-start:0;--background:transparent;--color:initial;display:-ms-flexbox;display:flex;position:relative;-ms-flex:1;flex:1;-ms-flex-align:center;align-items:center;width:100%;padding:0 !important;background:var(--background);color:var(--color);font-family:var(--stories-font-family, inherit);z-index:2}/*!@:host-context(stories-item:not(.item-label))*/stories-item.sc-stories-input-h:not(.item-label),stories-item:not(.item-label) .sc-stories-input-h{--padding-start:0}/*!@:host(.stories-color)*/.stories-color.sc-stories-input-h{color:var(--stories-color-base)}/*!@.native-input*/.native-input.sc-stories-input{border-radius:var(--border-radius);padding-left:var(--padding-start);padding-right:var(--padding-end);padding-top:var(--padding-top);padding-bottom:var(--padding-bottom);font-family:inherit;font-size:inherit;font-style:inherit;font-weight:inherit;letter-spacing:inherit;text-decoration:inherit;text-indent:inherit;text-overflow:inherit;text-transform:inherit;text-align:inherit;white-space:inherit;color:inherit;display:inline-block;-ms-flex:1;flex:1;width:100%;max-width:100%;max-height:100%;border:0;outline:none;background:transparent;-webkit-box-sizing:border-box;box-sizing:border-box;-webkit-appearance:none;-moz-appearance:none;appearance:none}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@.native-input*/.native-input.sc-stories-input{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--padding-start);padding-inline-start:var(--padding-start);-webkit-padding-end:var(--padding-end);padding-inline-end:var(--padding-end)}}/*!@.native-input::placeholder*/.native-input.sc-stories-input::-webkit-input-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.native-input.sc-stories-input::-moz-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.native-input.sc-stories-input:-ms-input-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.native-input.sc-stories-input::-ms-input-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.native-input.sc-stories-input::placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}/*!@.native-input:-webkit-autofill*/.native-input.sc-stories-input:-webkit-autofill{background-color:transparent}/*!@.native-input:invalid*/.native-input.sc-stories-input:invalid{-webkit-box-shadow:none;box-shadow:none}/*!@.native-input::-ms-clear*/.native-input.sc-stories-input::-ms-clear{display:none}/*!@.native-input[disabled]*/.native-input[disabled].sc-stories-input{opacity:0.4}/*!@.cloned-input*/.cloned-input.sc-stories-input{left:0;top:0;position:absolute;pointer-events:none}/*!@[dir=rtl] .cloned-input, :host-context([dir=rtl]) .cloned-input*/[dir=rtl].sc-stories-input .cloned-input.sc-stories-input,[dir=rtl].sc-stories-input-h .cloned-input.sc-stories-input,[dir=rtl] .sc-stories-input-h .cloned-input.sc-stories-input{left:unset;right:unset;right:0}/*!@.input-clear-icon*/.input-clear-icon.sc-stories-input{margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;padding-left:0;padding-right:0;padding-top:0;padding-bottom:0;background-position:center;border:0;outline:none;background-color:transparent;background-repeat:no-repeat;visibility:hidden;-webkit-appearance:none;-moz-appearance:none;appearance:none}/*!@.input-clear-icon:focus*/.input-clear-icon.sc-stories-input:focus{opacity:0.5}/*!@:host(.has-value) .input-clear-icon*/.has-value.sc-stories-input-h .input-clear-icon.sc-stories-input{visibility:visible}/*!@:host(.has-focus)*/.has-focus.sc-stories-input-h{pointer-events:none}/*!@:host(.has-focus) input,\n:host(.has-focus) a,\n:host(.has-focus) button*/.has-focus.sc-stories-input-h input.sc-stories-input,.has-focus.sc-stories-input-h a.sc-stories-input,.has-focus.sc-stories-input-h button.sc-stories-input{pointer-events:auto}/*!@:host-context(.item-label-floating.item-has-placeholder:not(.item-has-value))*/.item-label-floating.item-has-placeholder.sc-stories-input-h:not(.item-has-value),.item-label-floating.item-has-placeholder:not(.item-has-value) .sc-stories-input-h{opacity:0}/*!@:host-context(.item-label-floating.item-has-placeholder:not(.item-has-value).item-has-focus)*/.item-label-floating.item-has-placeholder.sc-stories-input-h:not(.item-has-value).item-has-focus,.item-label-floating.item-has-placeholder:not(.item-has-value).item-has-focus .sc-stories-input-h{-webkit-transition:opacity 0.15s cubic-bezier(0.4, 0, 0.2, 1);transition:opacity 0.15s cubic-bezier(0.4, 0, 0.2, 1);opacity:1}";
...
const labelCss = "/*!@:host-context(.item)*/.item.sc-stories-label-h,.item .sc-stories-label-h{--color:initial;display:block;color:var(--color);font-family:var(--stories-font-family, inherit);font-size:inherit;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;-webkit-box-sizing:border-box;box-sizing:border-box}/*!@:host(.stories-color)*/.stories-color.sc-stories-label-h{color:var(--stories-color-base)}/*!@:host(.stories-text-wrap),\n:host([text-wrap])*/.stories-text-wrap.sc-stories-label-h,[text-wrap].sc-stories-label-h{white-space:normal}/*!@:host-context(.item-interactive-disabled:not(.item-multiple-inputs))*/.item-interactive-disabled.sc-stories-label-h:not(.item-multiple-inputs),.item-interactive-disabled:not(.item-multiple-inputs) .sc-stories-label-h{cursor:default;opacity:0.3;pointer-events:none}/*!@:host-context(.item-input)*/.item-input.sc-stories-label-h,.item-input .sc-stories-label-h{-ms-flex:initial;flex:initial;max-width:200px;pointer-events:none}/*!@:host-context(.item-textarea)*/.item-textarea.sc-stories-label-h,.item-textarea .sc-stories-label-h{-ms-flex-item-align:baseline;align-self:baseline}/*!@:host(.label-fixed)*/.label-fixed.sc-stories-label-h{-ms-flex:0 0 100px;flex:0 0 100px;width:100px;min-width:100px;max-width:200px}/*!@:host(.label-stacked),\n:host(.label-floating)*/.label-stacked.sc-stories-label-h,.label-floating.sc-stories-label-h{margin-bottom:0;-ms-flex-item-align:stretch;align-self:stretch;width:auto;max-width:100%}/*!@:host(.label-no-animate.label-floating)*/.label-no-animate.label-floating.sc-stories-label-h{-webkit-transition:none;transition:none}/*!@::slotted(*) h1,\n::slotted(*) h2,\n::slotted(*) h3,\n::slotted(*) h4,\n::slotted(*) h5,\n::slotted(*) h6*/.sc-stories-label-s h1,.sc-stories-label-s h2,.sc-stories-label-s h3,.sc-stories-label-s h4,.sc-stories-label-s h5,.sc-stories-label-s h6{text-overflow:inherit;overflow:inherit}";
...
class Label {
  ...
  render() {
    ...
    return (hAsync(Host, { class: createColorClasses(this.color, {
...
const previewCss = "/*!@:host*/.sc-stories-preview-h{display:block;background-color:lightpink}/*!@section*/section.sc-stories-preview{-ms-flex-negative:1;flex-shrink:1;height:100vh;width:100vw;background-color:lightcoral}";
...
class Preview {
  ...
  render() {
    ...
    return (hAsync("stories-zoom", { zoom: zoom }, hAsync("slot", null)));
  }
  static get style() { return previewCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-preview",
    "$members$": undefined,
    "$listeners$": undefined,
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
...
const routerCss = "/*!@:host*/.sc-stories-router-h{display:block}";
...
const rowCss = "/*!@:host*/.sc-stories-row-h{display:-ms-flexbox;display:flex;-ms-flex-wrap:wrap;flex-wrap:wrap}";
...
const searchbarCss = "/*!@:host*/.sc-stories-searchbar-h{--placeholder-color:initial;--placeholder-font-style:initial;--placeholder-font-weight:initial;--placeholder-opacity:.5;-moz-osx-font-smoothing:grayscale;-webkit-font-smoothing:antialiased;display:-ms-flexbox;display:flex;position:relative;-ms-flex-align:center;align-items:center;width:100%;color:var(--color);font-family:var(--stories-font-family, inherit);-webkit-box-sizing:border-box;box-sizing:border-box}/*!@:host(.stories-color)*/.stories-color.sc-stories-searchbar-h{color:var(--stories-color-contrast)}/*!@:host(.stories-color) .searchbar-input*/.stories-color.sc-stories-searchbar-h .searchbar-input.sc-stories-searchbar{background:var(--stories-color-base)}/*!@:host(.stories-color) .searchbar-clear-button,\n:host(.stories-color) .searchbar-cancel-button,\n:host(.stories-color) .searchbar-search-icon*/.stories-color.sc-stories-searchbar-h .searchbar-clear-button.sc-stories-searchbar,.stories-color.sc-stories-searchbar-h .searchbar-cancel-button.sc-stories-searchbar,.stories-color.sc-stories-searchbar-h .searchbar-search-icon.sc-stories-searchbar{color:inherit}/*!@.searchbar-search-icon*/.searchbar-search-icon.sc-stories-searchbar{color:var(--icon-color);pointer-events:none}/*!@.searchbar-input-container*/.searchbar-input-container.sc-stories-searchbar{display:block;position:relative;-ms-flex-negative:1;flex-shrink:1;width:100%}/*!@.searchbar-input*/.searchbar-input.sc-stories-searchbar{font-family:inherit;font-size:inherit;font-style:inherit;font-weight:inherit;letter-spacing:inherit;text-decoration:inherit;text-indent:inherit;text-overflow:inherit;text-transform:inherit;text-align:inherit;white-space:inherit;color:inherit;border-radius:var(--border-radius);display:block;width:100%;border:0;outline:none;background:var(--background);font-family:inherit;-webkit-box-shadow:var(--box-shadow);box-shadow:var(--box-shadow);-webkit-box-sizing:border-box;box-sizing:border-box;-webkit-appearance:none;-moz-appearance:none;appearance:none}/*!@.searchbar-input::placeholder*/.searchbar-input.sc-stories-searchbar::-webkit-input-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.searchbar-input.sc-stories-searchbar::-moz-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.searchbar-input.sc-stories-searchbar:-ms-input-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.searchbar-input.sc-stories-searchbar::-ms-input-placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}.searchbar-input.sc-stories-searchbar::placeholder{color:var(--placeholder-color);font-family:inherit;font-style:var(--placeholder-font-style);font-weight:var(--placeholder-font-weight);opacity:var(--placeholder-opacity)}/*!@.searchbar-input::-webkit-search-cancel-button, .searchbar-input::-ms-clear*/.searchbar-input.sc-stories-searchbar::-webkit-search-cancel-button,.searchbar-input.sc-stories-searchbar::-ms-clear{display:none}/*!@.searchbar-cancel-button*/.searchbar-cancel-button.sc-stories-searchbar{margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;display:none;height:100%;border:0;outline:none;color:var(--cancel-button-color);cursor:pointer;-webkit-appearance:none;-moz-appearance:none;appearance:none}/*!@.searchbar-cancel-button > div*/.searchbar-cancel-button.sc-stories-searchbar>div.sc-stories-searchbar{display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;-ms-flex-pack:center;justify-content:center;width:100%;height:100%}/*!@.searchbar-clear-button*/.searchbar-clear-button.sc-stories-searchbar{margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;padding-left:0;padding-right:0;padding-top:0;padding-bottom:0;display:none;min-height:0;outline:none;color:var(--clear-button-color);-webkit-appearance:none;-moz-appearance:none;appearance:none}/*!@:host(.searchbar-has-value.searchbar-should-show-clear) .searchbar-clear-button*/.searchbar-has-value.searchbar-should-show-clear.sc-stories-searchbar-h .searchbar-clear-button.sc-stories-searchbar{display:block}/*!@:host(.searchbar-disabled)*/.searchbar-disabled.sc-stories-searchbar-h{cursor:default;opacity:0.4;pointer-events:none}";
...
class Searchbar {
  ...
  render() {
    ...
    return (hAsync(Host, { "aria-disabled": this.disabled ? 'true' : null, class: createColorClasses(this.color, {
      ...
      }), role: "search" }, hAsync("div", { class: "searchbar-input-container" }, hAsync("input", { ref: el => this.nativeInput = el, "aria-label": "search text", class: "searchbar-input", disabled: this.disabled, inputMode: this.inputmode, onBlur: this.onBlur, onFocus: this.onFocus, onInput: this.onInput, placeholder: this.placeholder, type: this.type, value: this.getValue() }), hAsync("stories-icon", { "aria-hidden": "true", class: "searchbar-search-icon", name: searchIcon }), hAsync("button", { "aria-label": "reset", class: "searchbar-clear-button", onMouseDown: ev => this.onClearInput(ev, true), onTouchStart: ev => this.onClearInput(ev, true), type: "button", "no-blur": true }, hAsync("stories-icon", { "aria-hidden": "true", class: "searchbar-clear-icon", name: clearIcon })))));
  }
  get el() { return getElement(this); }
  static get watchers() { return {
    "debounce": ["debounceChanged"],
...
const sidebarCss = "/*!@:host*/.sc-stories-sidebar-h{display:block;background-color:lightblue}/*!@nav*/nav.sc-stories-sidebar{-ms-flex-negative:0;flex-shrink:0;height:100vh;width:200px}";
...
class Sidebar {
  ...
  render() {
    ...
    if (stories && Object.keys(stories).length) {
      return (hAsync("nav", null, hAsync("h1", null, "Navigation"), Object.keys(stories).map(key => {
        const story = stories[key];
        return hAsync("li", null, hAsync("a", { href: `/#path=${key}` }, story.storyName));
      })));
    }
    return hAsync("div", null, "No stories");
  }
  static get style() { return sidebarCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-sidebar",
    "$members$": undefined,
    "$listeners$": undefined,
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
...
const splitPaneCss = "/*!@:host*/.sc-stories-split-pane-h{display:grid;grid-template-columns:1fr -webkit-max-content 1fr;grid-template-columns:1fr max-content 1fr;-webkit-user-select:auto;-moz-user-select:auto;-ms-user-select:auto;user-select:auto;cursor:auto;width:auto;height:100vh}/*!@:host([resizing])*/[resizing].sc-stories-split-pane-h{-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none}/*!@:host([resizing][split=horizontal])*/[resizing][split=horizontal].sc-stories-split-pane-h{cursor:col-resize}/*!@:host #median*/.sc-stories-split-pane-h #median.sc-stories-split-pane{inline-size:0.5rem;grid-column:2/3}/*!@:host #median:hover*/.sc-stories-split-pane-h #median.sc-stories-split-pane:hover{cursor:col-resize}/*!@:host [name=slot1]*/.sc-stories-split-pane-h [name=slot1].sc-stories-split-pane{grid-column:1/2;grid-row:1/1}/*!@:host [name=slot2]*/.sc-stories-split-pane-h [name=slot2].sc-stories-split-pane{grid-column:3/4;grid-row:1/1}/*!@:host([resizing][split=vertical])*/[resizing][split=vertical].sc-stories-split-pane-h{cursor:row-resize}/*!@:host([split=vertical])*/[split=vertical].sc-stories-split-pane-h{grid-template-rows:1fr -webkit-max-content 1fr;grid-template-rows:1fr max-content 1fr;grid-template-columns:none}/*!@:host([split=vertical]) #median*/[split=vertical].sc-stories-split-pane-h #median.sc-stories-split-pane{block-size:0.5rem;inline-size:auto;grid-row:2/3;grid-column:1/1}/*!@:host([split=vertical]) #median:hover*/[split=vertical].sc-stories-split-pane-h #median.sc-stories-split-pane:hover{cursor:row-resize}/*!@:host([split=vertical]) [name=slot1]*/[split=vertical].sc-stories-split-pane-h [name=slot1].sc-stories-split-pane{grid-row:1/2;grid-column:1/1}/*!@:host([split=vertical]) [name=slot2]*/[split=vertical].sc-stories-split-pane-h [name=slot2].sc-stories-split-pane{grid-row:3/4;grid-column:1/1}/*!@#median*/#median.sc-stories-split-pane{background:bisque}/*!@::slotted(*)*/.sc-stories-split-pane-s>*{overflow:auto}";
...
class SplitPane {
  ...
  render() {
    return [
      hAsync("slot", { name: "slot1" }),
      hAsync("div", { ref: el => this.median = el, id: "median", onPointerDown: this.pointerDown }),
      hAsync("slot", { name: "slot2" })
    ];
  }
  get el() { return getElement(this); }
  ...
  static get cmpMeta() { return {
    ...
    "$listeners$": undefined,
    "$lazyBundleId$": "-",
    "$attrsToReflect$": [["isResizing", "resizing"]]
  }; }
}

const tabCss = "/*!@:host(.tab-hidden)*/.tab-hidden.sc-stories-tab-h{display:none !important}";
...
class Tab {
  ...
  render() {
    ...
    return (hAsync(Host, { "aria-hidden": !active ? 'true' : null, "aria-labelledby": `tab-button-${tab}`, class: {
        'stories-page': true,
        'tab-hidden': !active
      }, role: "tabpanel" }, hAsync("slot", null)));
  }
  get el() { return getElement(this); }
...
const tabBarCss = "/*!@:host*/.sc-stories-tab-bar-h{padding-left:var(--stories-safe-area-left);padding-right:var(--stories-safe-area-right);display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;-ms-flex-pack:start;justify-content:flex-start;width:auto;height:40px;padding-bottom:var(--stories-safe-area-bottom, 0);border-top:var(--border);background:var(--background);color:var(--color);text-align:center;contain:strict;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;z-index:10;-webkit-box-sizing:content-box !important;box-sizing:content-box !important}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@:host*/.sc-stories-tab-bar-h{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--stories-safe-area-left);padding-inline-start:var(--stories-safe-area-left);-webkit-padding-end:var(--stories-safe-area-right);padding-inline-end:var(--stories-safe-area-right)}}/*!@:host(.stories-color) ::slotted(stories-tab-button)*/.sc-stories-tab-bar-h.stories-color .sc-stories-tab-bar-s>stories-tab-button{--background-focused:var(--stories-color-shade);--color-selected:var(--stories-color-contrast)}/*!@:host(.stories-color) ::slotted(.tab-selected)*/.sc-stories-tab-bar-h.stories-color .sc-stories-tab-bar-s>.tab-selected{color:var(--stories-color-contrast)}/*!@:host(.stories-color),\n:host(.stories-color) ::slotted(stories-tab-button)*/.stories-color.sc-stories-tab-bar-h,.sc-stories-tab-bar-h.stories-color .sc-stories-tab-bar-s>stories-tab-button{color:rgba(var(--stories-color-contrast-rgb), 0.7)}/*!@:host(.stories-color),\n:host(.stories-color) ::slotted(stories-tab-button)*/.stories-color.sc-stories-tab-bar-h,.sc-stories-tab-bar-h.stories-color .sc-stories-tab-bar-s>stories-tab-button{background:var(--stories-color-base)}/*!@:host(.stories-color) ::slotted(stories-tab-button.stories-focused)*/.sc-stories-tab-bar-h.stories-color .sc-stories-tab-bar-s>stories-tab-button.stories-focused{background:var(--background-focused)}/*!@:host(.tab-bar-hidden)*/.tab-bar-hidden.sc-stories-tab-bar-h{display:none !important}";
...
class TabBar {
  ...
  render() {
    const { color } = this;
    return (hAsync(Host, { class: createColorClasses(color, {}), role: "tablist" }, hAsync("slot", null)));
  }
  get el() { return getElement(this); }
  static get watchers() { return {
...
const tabButtonCss = "/*!@:host*/.sc-stories-tab-button-h{--background-focused-opacity:1;-ms-flex-align:center;align-items:center;-ms-flex-pack:center;justify-content:center;height:100%;outline:none;background:pink;color:var(--color)}/*!@.button-native*/.button-native.sc-stories-tab-button{border-radius:inherit;margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;padding-left:var(--padding-start);padding-right:var(--padding-end);padding-top:var(--padding-top);padding-bottom:var(--padding-bottom);font-family:inherit;font-size:inherit;font-style:inherit;font-weight:inherit;letter-spacing:inherit;text-decoration:inherit;text-indent:inherit;text-overflow:inherit;text-transform:inherit;text-align:inherit;white-space:inherit;color:inherit;display:-ms-flexbox;display:flex;position:relative;-ms-flex-direction:inherit;flex-direction:inherit;-ms-flex-align:inherit;align-items:inherit;-ms-flex-pack:inherit;justify-content:inherit;height:100%;border:0;outline:none;background:transparent;text-decoration:none;cursor:pointer;overflow:hidden;-webkit-box-sizing:border-box;box-sizing:border-box;-webkit-user-drag:none}@supports ((-webkit-margin-start: 0) or (margin-inline-start: 0)) or (-webkit-margin-start: 0){/*!@.button-native*/.button-native.sc-stories-tab-button{padding-left:unset;padding-right:unset;-webkit-padding-start:var(--padding-start);padding-inline-start:var(--padding-start);-webkit-padding-end:var(--padding-end);padding-inline-end:var(--padding-end)}}/*!@.button-native::after*/.button-native.sc-stories-tab-button::after{left:0;right:0;top:0;bottom:0;position:absolute;content:\"\";opacity:0}/*!@.button-inner*/.button-inner.sc-stories-tab-button{display:-ms-flexbox;display:flex;position:relative;-ms-flex-flow:inherit;flex-flow:inherit;-ms-flex-align:inherit;align-items:inherit;-ms-flex-pack:inherit;justify-content:inherit;height:100%;z-index:1}/*!@:host(.stories-focused) .button-native*/.stories-focused.sc-stories-tab-button-h .button-native.sc-stories-tab-button{color:var(--color-focused)}/*!@:host(.stories-focused) .button-native::after*/.stories-focused.sc-stories-tab-button-h .button-native.sc-stories-tab-button::after{background:var(--background-focused);opacity:var(--background-focused-opacity)}@media (any-hover: hover){/*!@a:hover*/a.sc-stories-tab-button:hover{color:var(--color-selected)}}/*!@:host(.tab-selected)*/.tab-selected.sc-stories-tab-button-h{color:var(--color-selected)}/*!@:host(.tab-hidden)*/.tab-hidden.sc-stories-tab-button-h{display:none !important}/*!@:host(.tab-disabled)*/.tab-disabled.sc-stories-tab-button-h{pointer-events:none;opacity:0.4}/*!@::slotted(stories-label),\n::slotted(stories-icon)*/.sc-stories-tab-button-s>stories-label,.sc-stories-tab-button-s>stories-icon{display:block;-ms-flex-item-align:center;align-self:center;max-width:100%;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;-webkit-box-sizing:border-box;box-sizing:border-box}/*!@::slotted(stories-label)*/.sc-stories-tab-button-s>stories-label{-ms-flex-order:0;order:0}/*!@::slotted(stories-icon)*/.sc-stories-tab-button-s>stories-icon{-ms-flex-order:-1;order:-1;height:1em}/*!@:host(.tab-has-label-only) ::slotted(stories-label)*/.sc-stories-tab-button-h.tab-has-label-only .sc-stories-tab-button-s>stories-label{white-space:normal}/*!@::slotted(stories-badge)*/.sc-stories-tab-button-s>stories-badge{-webkit-box-sizing:border-box;box-sizing:border-box;position:absolute;z-index:1}/*!@:host(.tab-layout-icon-start)*/.tab-layout-icon-start.sc-stories-tab-button-h{-ms-flex-direction:row;flex-direction:row}/*!@:host(.tab-layout-icon-end)*/.tab-layout-icon-end.sc-stories-tab-button-h{-ms-flex-direction:row-reverse;flex-direction:row-reverse}/*!@:host(.tab-layout-icon-bottom)*/.tab-layout-icon-bottom.sc-stories-tab-button-h{-ms-flex-direction:column-reverse;flex-direction:column-reverse}/*!@:host(.tab-layout-icon-hide) ::slotted(stories-icon)*/.sc-stories-tab-button-h.tab-layout-icon-hide .sc-stories-tab-button-s>stories-icon{display:none}/*!@:host(.tab-layout-label-hide) ::slotted(stories-label)*/.sc-stories-tab-button-h.tab-layout-label-hide .sc-stories-tab-button-s>stories-label{display:none}";
...
class TabButton {
  ...
  render() {
    ...
    return (hAsync(Host, { "aria-selected": selected ? 'true' : null, class: {
      ...
      }, id: tab !== undefined ? `tab-button-${tab}` : null, onClick: this.onClick, onKeyup: this.onKeyUp, role: "tab", tabindex: tabIndex }, hAsync("a", { href: '#', tabIndex: -1, class: "button-native", part: "native" }, hAsync("span", { class: "button-inner" }, hAsync("slot", null)))));
  }
  get el() { return getElement(this); }
  static get style() { return tabButtonCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-tab-button",
    "$members$": {
      "disabled": [4],
      "layout": [1025],
      "selected": [1028],
    ...
    "$listeners$": [[8, "storiesTabBarChanged", "onTabBarChanged"]],
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
  }; }
}

const tabsCss = "/*!@:host*/.sc-stories-tabs-h{display:-ms-flexbox;display:flex;position:relative;-ms-flex-direction:column;flex-direction:column;width:100%;height:100%;contain:layout size style;z-index:0}/*!@.tabs-inner*/.tabs-inner.sc-stories-tabs{position:relative;-ms-flex:1;flex:1;contain:layout size style}";
...
class Tabs {
  ...
  render() {
    return (hAsync(Host, { onStoriesTabButtonClick: this.onTabClicked }, hAsync("div", { class: "tabs-inner" }, hAsync("slot", null))));
  }
  get el() { return getElement(this); }
  static get style() { return tabsCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-tabs",
    "$members$": {
      "selectedTab": [32],
      "select": [64],
      "getTab": [64],
...
const toolBarCss = "/*!@.bar*/.bar.sc-stories-tool-bar{color:var(--stories-bar-text-color, black);width:100%;height:40px;-ms-flex-negative:0;flex-shrink:0;overflow:auto;overflow-y:hidden}/*!@.inner*/.inner.sc-stories-tool-bar{display:-ms-flexbox;display:flex;-ms-flex-pack:justify;justify-content:space-between;position:relative;-ms-flex-wrap:nowrap;flex-wrap:nowrap;-ms-flex-negative:0;flex-shrink:0;height:40px;background-color:transparent}/*!@.side*/.side.sc-stories-tool-bar{display:-ms-flexbox;display:flex;white-space:nowrap;-ms-flex-preferred-size:auto;flex-basis:auto;-ms-flex-negative:0;flex-shrink:0;margin-left:3px;margin-right:3px}/*!@.side.left > **/.side.left.sc-stories-tool-bar>*.sc-stories-tool-bar{margin-left:4px}/*!@.side.right*/.side.right.sc-stories-tool-bar{margin-left:30px}/*!@.side.right > **/.side.right.sc-stories-tool-bar>*.sc-stories-tool-bar{margin-right:4px}";
...
class ToolBar {
  ...
  render() {
    return (hAsync("div", { class: "bar" }, hAsync("div", { class: "inner" }, hAsync("div", { class: "side left" }, hAsync("slot", { name: "left" })), hAsync("div", { class: "side right" }, hAsync("slot", { name: "right" })))));
  }
  static get style() { return toolBarCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-tool-bar",
    "$members$": undefined,
    "$listeners$": undefined,
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
...
const toolButtonCss = "/*!@button*/button.sc-stories-tool-button{-ms-flex-align:center;align-items:center;background:transparent;border:none;border-radius:4px;color:inherit;cursor:pointer;display:-ms-inline-flexbox;display:inline-flex;font-size:13px;font-weight:bold;height:28px;-ms-flex-pack:center;justify-content:center;margin-top:6px;padding:8px 7px}/*!@button:hover, button:focus-visible*/button.sc-stories-tool-button:hover,button.sc-stories-tool-button:focus-visible{color:var(--color-hover);background:var(--background-hover);opacity:var(--background-hover-opacity)}/*!@button:focus-visible*/button.sc-stories-tool-button:focus-visible{outline:auto}/*!@button:focus:not(:focus-visible)*/button.sc-stories-tool-button:focus:not(:focus-visible){outline:\"none\"}/*!@button.active*/button.active.sc-stories-tool-button{color:var(--color-hover);background:var(--background-hover)}/*!@button.disabled*/button.disabled.sc-stories-tool-button{opacity:0.5;cursor:not-allowed}";
...
class ToolButton {
  ...
  render() {
    return (hAsync("button", { disabled: this.disabled, onClick: this.clickHandler.bind(this) }, hAsync("stories-icon", { name: this.icon })));
  }
  static get style() { return toolButtonCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-tool-button",
    "$members$": {
      "disabled": [4],
      "icon": [1],
      "command": [1]
...
const toolZoomCss = "/*!@:host*/.sc-stories-tool-zoom-h{display:block}";
...
class ToolZoom {
  ...
  render() {
    return [
      hAsync("stories-tool-button", { command: "zoomIn", icon: "zoomIn", onStoriesAction: this.actionHandler }),
      hAsync("stories-tool-button", { command: "zoomOut", icon: "zoomOut", onStoriesAction: this.actionHandler }),
      hAsync("stories-tool-button", { command: "zoomReset", icon: "zoomReset", onStoriesAction: this.actionHandler }),
    ];
  }
  static get style() { return toolZoomCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "stories-tool-zoom",
    "$members$": undefined,
    "$listeners$": undefined,
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
...
const zoomCss = "/*!@:host*/.sc-stories-zoom-h{display:block}";
...
class Zoom {
  ...
  render() {
    ...
    return (hAsync(Host, { style: style }, hAsync("div", { ref: (el) => this.div = el, class: "innerZoomElementWrapper" }, hAsync("slot", null))));
  }
  static get watchers() { return {
