# stories-actionbutton



<!-- Auto Generated Below -->


## Properties

| Property                  | Attribute | Description         | Type                                                                                   | Default     |
| ------------------------- | --------- | ------------------- | -------------------------------------------------------------------------------------- | ----------- |
| `actionItem` _(required)_ | --        | ActionItem property | `{ command: string; title: string \| Element; disabled?: boolean; active?: boolean; }` | `undefined` |


## Events

| Event           | Description  | Type                       |
| --------------- | ------------ | -------------------------- |
| `storiesAction` | Action Event | `CustomEvent<ActionEvent>` |


## Dependencies

### Used by

 - [stories-action-bar](../action-bar)

### Graph
```mermaid
graph TD;
  stories-action-bar --> stories-action-button
  style stories-action-button fill:#f9f,stroke:#333,stroke-width:4px
```

----------------------------------------------

*Built with [StencilJS](https://stenciljs.com/)*
